...
function DataSourceWrapper(props: withAppTypes) {
  ...
  const getInitialDataSourceName = useCallback(() => {
    ...
    if (!dataSourceName) {
      ...
      const webApiDataSources = dataSourceModules.reduce((acc, curr) => {
        const mods = curr.module.filter(mod => mod.type === 'webApi');
        return acc.concat(mods);
      }, []);
      dataSourceName = webApiDataSources
        .map(ds => ds.name)
        .find(name => extensionManager.getDataSources(name)?.[0] !== undefined);
    }
  ...
  const [dataSource, setDataSource] = useState(() => {
    ...
    if (!dataSourceName) {
      ...
      dataSource = activeDataSource && activeDataSource[0];
    } else {
      dataSource = extensionManager.getDataSources(dataSourceName)?.[0];
    }

    if (!dataSource) {
      console.warn(
        `No valid data source found for ${dataSourceName || 'active data source'}. Falling back to first available data source.`
      );
      const allDataSources = extensionManager.getDataSources();
      dataSource = allDataSources && allDataSources[0]?.[0]; // Fallback to first available data source
    }

    if (!dataSource) {
      throw new Error(
        `No data source available. Check configuration or extensionManager initialization.`
      );
    }
    ...
    const initializeDataSource = async () => {
      try {
        ...
        servicesManager.services.uiModalService.show({
          title: 'Data Source Initialization Error',
          content: () => (
            <div className="text-foreground">
              <p className="text-red-600">Error: {error.message}</p>
              <p>
                Failed to initialize the data source: {dataSource.getConfig().friendlyName}. Please
  ...
  useEffect(() => {
    ...
    async function getData() {
      ...
      try {
        ...
        console.log(studies, 'studies00000000000000000000');
        setData({
          studies: Array.isArray(studies) ? studies : [],
          total: Array.isArray(studies) ? studies.length : 0,
